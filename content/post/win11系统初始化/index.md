...
## 1.4 开启开发者模式（mklink软链接/WSL/Docker必备）
设置→隐私和安全性→开发者选项→打开**开发人员模式**，确认开启设备门户、符号链接权限。

## 1.5 磁盘空间释放（SSD必做，实测释放数十GB）
管理员PowerShell逐条执行：
```powershell
# 清理系统更新冗余缓存
...
## 1.7 Defender杀毒深度优化（解决编译卡顿、Docker扫描卡死，全网实测最优方案）
### 1.7.1 批量添加开发目录排除（管理员PowerShell执行）
```powershell
# 全局开发根目录
...
```
### 1.7.2 Dev Drive开发盘性能模式（Win11 22H2+，新增全网高分优化）
...
## 3.1 EXE/MSI系统安装软件（默认C盘Program Files，不改安装路径）
...
## 3.4 Docker Desktop 外置数据完整配置（解决C盘爆满核心痛点，实测踩坑修复）
...
## 4.1 方案1：Batch菜单脚本（临时终端切换，新手首选，关闭终端自动重置）
...
## 4.2 方案2：PowerShell全局持久切换（Windows Terminal主力用户）
...
## 4.3 方案3：软链接全局切换（全系统所有软件统一版本，管理员运行）
...
## 6.1 一键安装Oh My Posh（管理员PowerShell）
```powershell
winget install JanDeDobbeleer.OhMyPosh
```
## 6.2 安装JetBrainsMono Nerd Font字体（显示Git图标、环境标识）
## 6.3 写入PowerShell配置自动加载主题
```powershell
...
## 8.1 PG文件索引基础表（Docker pgvector库）
```sql
